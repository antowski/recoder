...
Процедура ПерекодироватьФайл(Знач пИмяФайла, ИсходнаяКодировка, ЦелеваяКодировка) Экспорт

	Ожидаем.Что(ФС.ФайлСуществует(пИмяФайла), "Не найден файл <" + пИмяФайла + ">").ЭтоИстина();

	Лог.Отладка("Выполняется перекодировка файла <" + пИмяФайла + "> из " + ИсходнаяКодировка + " в " + ЦелеваяКодировка);

	Если НРег(ЦелеваяКодировка) = НРег(ИсходнаяКодировка) Тогда
		Возврат;
	КонецЕсли;

	Если НРег(ЦелеваяКодировка) = "utf-8" И ЭтоФайлВUTF8withBOM(пИмяФайла) Тогда
		Возврат;
	КонецЕсли;
...
// Например: ".txt,.log"
Процедура ПерекодироватьКаталог(Знач ИмяКаталога, Знач МаскаПоиска = "", ИсходнаяКодировка = "windows-1251", ЦелеваяКодировка = "UTF-8") Экспорт
	...
	Для каждого НайденныйФайл Из НайденныеФайлы Цикл
		...
		Если ОбрабатыватьФайл(НайденныйФайл, МассивОбрабатываемыхРасширений) Тогда
			...
			ПерекодироватьФайл(НайденныйФайл.ПолноеИмя, ИсходнаяКодировка, ЦелеваяКодировка);
		КонецЕсли;
...
Процедура ЗаписатьТекстВКодировке(пТекст, пИмяФайла, пКодировкаСтрокой)
	...
	Исключение		
		Писатель.Закрыть();
		ВызватьИсключение "Ошибка при записи файла <" + пИмяФайла + ">: " 
